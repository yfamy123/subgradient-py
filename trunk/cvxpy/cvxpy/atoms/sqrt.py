...
class cvxpy_sqrt(object):
    ...
    def _construct(self,args,t2,mp,n):
        #t1 = args[0]
        #return square.construct([t2],t1,mp,n)
        ...
        # f
        def f(x):
            return x[mp[t2]] - np.sqrt(x[mp[t1]])

        # grad f
        def grad_f(x):
            g = opt.spmatrix(0.0,[],[],(n,1))
            g[mp[t1]] = -0.5/(np.sqrt(x[mp[t1]]))
            g[mp[t2]] = 1.0
            return g
        ...
        def hess_f(x):
            ...
            h[mp[t1],mp[t1]] = 0.25/(np.power(x[mp[t1]],3./2.))
            return h
    ...
    def _linearize(self,args,t2):
        t1 = args[0]
        return square._linearize([t2],t1)
