...
% setup plot data
iters = [1:MAX_ITERS];
fbest_nom1 = hist1{1}; fbest1 = hist1{2}; xs1 = hist1{3};
fbest_nom2 = hist2{1}; fbest2 = hist2{2}; xs2 = hist2{3};
fbest_nom3 = hist3{1}; fbest3 = hist3{2}; xs3 = hist3{3};
fbest_nomo = histo{1}; fbesto = histo{2}; xso = histo{3};
...
figure(1), clf
set(gca, 'FontSize',18);
semilogy( iters, fbest(fbest1_mc)-f_min, 'b--','LineWidth',1.5 ), hold on,
semilogy( iters, fbest(fbest2_mc)-f_min, 'k:','LineWidth',1.5 );
semilogy( iters, fbest(fbest3_mc)-f_min, 'r-.','LineWidth',1.5 );
hold off
axis([1 2000 1e-3 2])
xlabel('k');
ylabel('fbest - fmin');
legend('K = 1','K = 10','K = 100 label',1);
% if( PRINT_PLOTS ), print -depsc expected_avgpwl_fbest, end
...
axis([common_axis])
title('K = 100')
if( PRINT_PLOTS ), print -depsc expected_pwl_fbest_dist, end
